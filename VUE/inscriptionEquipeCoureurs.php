...
foreach ($listeAss as $uneAss){
	// pour TESTER : echo "UNE ASSOCIATION : </br>";print_r($uneAss);
				$pageInscriptionEquipeCoureurs->contenu .= '<label class="radio-inline"><input type="radio" id='.$uneAss->IDASS.' value='.$uneAss->IDASS.' name="radioASS" required>'.$uneAss->NOMASS.'</label>';
}
...
$pageInscriptionEquipeCoureurs->contenu .= '<section>
		<article>
			<form class="form" id="formInscriptionCoureurs" method="post" action="../CONTROLEUR/tt_AjoutCoureursEquipe.php">
				<div class="form-group">
